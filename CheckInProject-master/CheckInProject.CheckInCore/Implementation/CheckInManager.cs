...
namespace CheckInProject.CheckInCore.Implementation
{
    public class CheckInManager : ICheckInManager
    {
        ...
        public async Task<List<CheckInDataModels>> GetTodayCheckInData()
        {
            return await Task.Run(() =>
            {
                return CheckInDatabaseService.CheckInData.AsEnumerable()
                    .Where(t => t.CheckInDate == DateOnly.FromDateTime(DateTime.Now))
                    .ToList();
            });
        }
        ...
        public List<CheckInDataExportModels> QueryTodayRecords()
        {
            ...
            {
                var personData = PersonDatabaseService.PersonData.AsEnumerable().Where(t => t.StudentID == item.StudentID).FirstOrDefault();
                if (personData != null)
                {
